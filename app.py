...
def sovereign_engine(data):
    # إزالة الفواصل قبل المعالجة لضمان سلامة البيانات
...
st.subheader("وحدة الترجمة الرياضية والمعالجة السيادية - إصدار البراءة")
...
c3.metric("حالة المترجم", "يدعم 'اس'", "Ready")

# --- 3. خانة البحث والمترجم الذكي (يدعم العربية) ---
raw_input = st.text_input("أدخل القيمة (يدعم الصيغة الأسية مثل 10^3 أو 10اس3):", placeholder="مثال: 10اس6")

final_value = raw_input
if raw_input:
    # المترجم يفحص وجود علامة ^ أو كلمة "اس"
    separator = None
    if "^" in raw_input: separator = "^"
    elif "اس" in raw_input: separator = "اس"
    
    if separator:
        try:
            base, exp = raw_input.split(separator)
            computed_val = int(base) ** int(exp)
            final_value = "{:,}".format(computed_val)
            st.success(f"✅ المترجم الرياضي: تم تحويل القيمة إلى {final_value}")
        except:
            st.error("⚠️ صيغة رياضية غير صحيحة - استخدم أرقاماً فقط (مثال: 10اس3)")
    ...
    if raw_input:
        ...
        st.markdown(f"""
            <div style="border: 2px solid #D4AF37; padding: 25px; border-radius: 15px; background-color: #1a1a1a; text-align: center; margin-top: 20px;">
                ...
                <h1 style="color: #ffffff; letter-spacing: 2px; font-size: 2.5em;">{output}</h1>
                <p style="color: #D4AF37; font-size: 0.9em; margin-top: 10px;">القيمة المعالجة: {final_value}</p>
            </div>
        """, unsafe_allow_html=True)
        st.rerun()
    else:
...
st.divider()
st.caption("نظام أوكورت السيادي | وحدة الترجمة الآلية لبيانات براءة الاختراع")
